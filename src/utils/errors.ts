...
export class RaukValidationError extends RaukError {
    ...
    getErrorsForProperty(propertyPath: string): ValidationErrorDetail[] {
        const findErrors = (errors: ValidationErrorDetail[], path: string): ValidationErrorDetail[] => {
            return errors
                .filter(error => error.property === path)
                .concat(...errors.flatMap(error =>
 ...
 */
export function isAuthenticationError(error: any): error is RaukAuthenticationError {
